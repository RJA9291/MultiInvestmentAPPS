...
use App\Modules\Investor\Application\Services\RejectAccessRequestService;
...
/** Proposed API-026's reject counterpart: POST /v1/access-requests/{id}/reject */
class RejectAccessRequestController
{
    public function __invoke(string $id, DecideAccessRequestRequest $request, RejectAccessRequestService $service)
    {
        $validated = $request->validated();

        try {
            $accessRequest = $service->execute($id, $validated['decided_by_user_id']);
